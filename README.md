...
t101 loading                       # → 加载画面 10 人信息（召唤师/英雄/位置/段位/等级）
t101 lcu status                    # → 检测客户端连接与当前阶段
t101 web                           # → 启动本地 Web 界面（浏览器打开）
t101 cache clear                   # → 清缓存
```
...
- 加载画面：10 人召唤师名/英雄/位置/段位/等级 → `t101 loading` 或 Web「🎮 对局」加载视图
- 游戏内（海斗）：3/7/11/14 级海克斯选牌 S/A/B/C/D 评级 → `t101 augment 牌名,...` 或 Web 游戏视图
...
| 面板 | 功能 |
|---|---|
| 👥 好友状态 | 在线好友近期 10 场战绩（胜率/KDA/状态分）+ 组队建议（🟢适合组队/🟡看情况/🔴谨慎），30s 自动刷新 |
