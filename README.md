...
`101-tools` 是面向《英雄联盟》国服的对局分析工具，命令行和本地 Web 面板共用同一套服务层。它读取 101.qq.com 的公开统计数据，以及本机 League Client 的只读 LCU 接口，为 BP、海克斯大乱斗和加载画面提供参考信息。

> 当前首发版本：`0.0.1`
> 仅用于个人学习和对局参考。请遵守《英雄联盟用户协议》以及腾讯/拳头相关规定。
